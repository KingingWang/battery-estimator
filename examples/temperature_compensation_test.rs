...
fn show_temperature_impact(chemistry: BatteryChemistry, min_v: f32, max_v: f32) {
    ...
    match estimator.estimate_soc(mid_voltage) {
        Ok(base_soc) => {
            println!("Temperature impact at {:.2}V (base SOC: {:.1}%):", mid_voltage, base_soc);
...
fn detailed_temperature_analysis() {
        ...
        match estimator.estimate_soc(*voltage) {
            Ok(base_soc) => {
                ...
                for temp in test_temperatures.iter() {
                    ...
                    println!("  {:4.0}°C: {:.1}% ({:+.1}%, {:.1}% relative change)", 
                        temp, compensated, diff, (diff / base_soc) * 100.0);
                }
                println!();
            }
    ...
    for temp in example_temps.iter() {
        ...
        println!("    Bounded compensation: {:.2}% (max ±5%)", bounded_compensation);
...
fn test_extreme_temperatures() {
    ...
    match estimator.estimate_soc(voltage) {
        Ok(base_soc) => {
            println!("Testing extreme temperatures at {:.1}V (base SOC: {:.1}%):", voltage, base_soc);
    ...
    match estimator.estimate_soc(voltage) {
        Ok(base_soc) => {
            ...
            for (temp, desc) in test_cases.iter() {
                ...
                println!("  {:>6.0}°C: {} -> Change: {:.1}% ({:.1}%)", 
                    temp, desc, change, change_percent);
            }
            
            println!();
            println!("Note: Temperature compensation is clamped to ±5% to prevent unrealistic values.");
        }
        Err(e) => println!("Error: {}", e),
    }
}
